...
public class ShowRanking : MonoBehaviour
{
    ...
    {
        //192~.5はローカルホストもしくはサーバ名（サーバのhtdocsフォルダ）に置き換える
        string url = "http://192.168.12.5/FPS/get_ranking.py";
    ...
    void makeRankingData()
    {
        ...
        for (int i = 0; i < userScores.Length; i++)
        {
            ...
            scoreText.text = $"{i + 1:000}位 UserName: {userScore.user_name} \nScore: {userScore.score}  Tag: {userScore.kill_cnt}  Time: {minutes}:{seconds}";
        }
    }
